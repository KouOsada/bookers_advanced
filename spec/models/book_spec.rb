...
RSpec.describe Book, "モデルに関するテスト", type: :model do
  ...
  context "空白のバリデーションチェック" do
    it "titleが空白の場合にバリデーションチェックされ空白のエラーメッセージがが返ってきているか" do
      book = Book.new(title: '', body: 'hoge')
      expect(book).to be_invalid
      expect(book.errors[:title]).to include("can't be blank")
    end
    it "bodyが空白の場合にバリデーションチェックされ空白のエラーメッセージがが返ってきているか" do
...
#       it 'N:1となっている' do
#         expect(Book.reflect_on_assosiation(:user).macro).to eq :belongs_to
#       end
#     end
#   end
# end